        ...
        try:
            ...
            for agent in agents:
                ...
                for goal in goals:
                    ...
                    # Filter progress tracking by timeframe
                    recent_progress = [
                        pt for pt in progress_tracking
                        if pt.get("date") and pt["date"] >= start_date
                    ]
                    
                    # Filter mood trend by timeframe  
                    recent_moods = [
                        mt for mt in mood_trend
                        if mt.get("date") and mt["date"] >= start_date
                    ]
                    
                    # Calculate progress percentage from latest progress_tracking entry
                    progress_percentage = 0.0
                    latest_loneliness_score = None
                    latest_social_score = None
                    
                    if progress_tracking:
                        latest_progress = max(progress_tracking, key=lambda x: x.get("date", datetime.min))
                        # Calculate progress based on scores (higher social_engagement_score = better progress)
                        social_score = latest_progress.get("social_engagement_score", 0)
                        loneliness_score = latest_progress.get("loneliness_score", 10)  # Default high loneliness
                        
                        # Calculate progress as improvement (lower loneliness + higher social engagement)
                        if social_score is not None:
                            progress_percentage = min(100, (social_score / 10.0) * 100)
                        
                        latest_loneliness_score = loneliness_score
                        latest_social_score = social_score
                    progress_percentage = 0.0
                    if progress_tracking:
